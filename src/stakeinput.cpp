...
// The zVLS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zVLS is a hash of the serial
...
{
    //Create an output returning the zVLS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZVLSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zVLS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zVLS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZVLSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zVLS output", __func__);
...
{
    CzVLSTracker* zvlsTracker = pwallet->zvlsTracker.get();
    CMintMeta meta;
    if (!zvlsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zvlsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a VLS stake is the outpoint
