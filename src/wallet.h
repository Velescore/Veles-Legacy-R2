...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1200 VLS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zVLS send
enum ZerocoinSpendStatus {
    ZVLS_SPEND_OKAY = 0,                            // No error
    ZVLS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVLS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVLS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVLS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVLS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZVLS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZVLS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZVLS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZVLS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZVLS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZVLS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZVLS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZVLS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVLS_SPENT_USED_ZVLS = 14,                      // Coin has already been spend
    ZVLS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZVLS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZVLSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzvlsAuto) const;
    ...
    */
    ...
    CzVLSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzVLSTracker> zvlsTracker;
    ...
    void setZWallet(CzVLSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zvlsTracker = std::unique_ptr<CzVLSTracker>(new CzVLSTracker(strWalletFile));
    }

    CzVLSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zVLS reset */
    boost::signals2::signal<void()> NotifyzVLSReset;
