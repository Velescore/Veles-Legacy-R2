...
#ifndef VELES_ZVLSWALLET_H
#define VELES_ZVLSWALLET_H
...
public:
    CzVLSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZVLS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZVLS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //VELES_ZVLSWALLET_H
