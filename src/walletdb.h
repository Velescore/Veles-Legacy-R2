...
// Copyright (c) 2016-2018 The VELES developers
...
public:
    ...
    bool WriteZVLSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVLSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVLSSeed_deprecated(uint256& seed);
    bool EraseZVLSSeed();
    bool EraseZVLSSeed_deprecated();

    bool WriteZVLSCount(const uint32_t& nCount);
    bool ReadZVLSCount(uint32_t& nCount);
