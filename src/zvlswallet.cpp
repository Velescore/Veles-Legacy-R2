...
CzVLSWallet::CzVLSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvls seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVLSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVLSSeed_deprecated()) {
                    LogPrintf("%s: Updated zVLS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvls seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvls wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVLSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVLSCount(nCountLastUsed);
    else if (!walletdb.ReadZVLSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVLSWallet::Lock()
{
    seedMaster = 0;
}

void CzVLSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVLSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVLS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVLSWallet::LoadMintPoolFromDB()
{
...
void CzVLSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVLSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVLSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvlsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVLSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVLS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvlsTracker which also adds to database
    pwalletMain->zvlsTracker->Add(dMint, true);
...
void CzVLSWallet::SeedToZVLS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVLSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVLSWallet::GenerateDeterministicZVLS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVLSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVLSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
