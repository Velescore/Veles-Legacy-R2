...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VLS);
    ...
    nAnonymizeVelesAmount = settings.value("nAnonymizeVelesAmount").toLongLong();
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in veles.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVelesAmount:
            return QVariant(nAnonymizeVelesAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVelesAmount:
            nAnonymizeVelesAmount = value.toInt();
            settings.setValue("nAnonymizeVelesAmount", nAnonymizeVelesAmount);
            emit anonymizeVelesAmountChanged(nAnonymizeVelesAmount);
            break;
        case CoinControlFeatures:
