...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered veles address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 VLS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal veles:URI (veles:XyZ...?message=example)
            {
...
static void NotifyzVLSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzVLSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzVLSReset.connect(boost::bind(NotifyzVLSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzVLSReset.disconnect(boost::bind(NotifyzVLSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zvlsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
