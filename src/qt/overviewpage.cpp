...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VLS)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sVLSPercentage, QString& szVLSPercentage)
{
    int nPrecision = 2;
    ...
    szVLSPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sVLSPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelVLSPercent->setText(sPercentage);
    ui->labelzVLSPercent->setText(szPercentage);
    ...
    ui->labelzBalanceMature->setVisible(showzVLSAvailable);
    ui->labelzBalanceMatureText->setVisible(showzVLSAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzVLSUnconfirmed);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("VLS")
    updateDisplayUnit();
}
