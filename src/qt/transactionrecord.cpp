...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zvlsTracker->HasMintTx(hash))) {
            //zVLS stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZVLS;
            ...
            // VLS stake reward
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            ExtractDestination(wtx.vout[2].scriptPubKey, address);
            ...
            // dev reward
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            ExtractDestination(wtx.vout[3].scriptPubKey, address);
            sub.type = TransactionRecord::MNReward;
            sub.address = CBitcoinAddress(address).ToString();
...
bool IsZVLSType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZVLS:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZVLS || type == TransactionRecord::MNReward) {
