...
int nAnonymizeVelesAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "veles" is a composite category enabling all VELES-related debug output
            if (ptrCategory->count(string("veles"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "veles.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty veles.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override veles.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "velesd.pid"));
